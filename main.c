...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
//***********GPIOA RELATED POINTERS********
volatile uint32_t* RCC_AHB1ENR=(volatile uint32_t*)0X40023800;
...
volatile uint32_t* TIM2_CCR1=(volatile uint32_t*)0X40000034;
volatile uint32_t* TIM2_CCER=(volatile uint32_t*)0X40000020;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  *RCC_AHB1ENR|=(1U<<0);//CLOCK ENABLE FOR GPIOA
  *RCC_APB1ENR|=(1U<<0);//TIM2 CLOCK ENABLE
  *GPIOA_MODER&=~(3U<<10);
  *GPIOA_MODER|=(1U<<11);//PA5 ALTERNATE FUNCTION MODE
  *GPIOA_AFRL&=~((1U<<20)|(1U<<21)|(1U<<22)|(1U<<23));
  *GPIOA_AFRL|=(1U<<20);
  *NVIC_ISER0|=(1U<<28);//TIM2 INTERRUPTS ENABLED TO MANAGE
  *TIM2_PSC=0X00015999;
  *TIM2_ARR=0X00000999;
  *TIM2_CCR1=0X00000500;//50% DUTY CYCLE
  *TIM2_CCMR1|=(6U<<4);
  *TIM2_CCER|=(1U<<0);
  *TIM2_CCMR1|=(1U<<3);
  *TIM2_CR1|=(1U<<0);
  *GPIOA_BSRR |= (1U<<5);   // force PA5 HIGH directly
